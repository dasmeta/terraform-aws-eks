...
| <a name="input_configs_crds"></a> [configs\_crds](#input\_configs\_crds) | Configurations to pass and override defaults for the linkerd-crds Helm chart. The module defaults installGatewayAPI to true (the chart itself ships it as false); set installGatewayAPI = false here where another component already owns the Gateway API CRDs in the cluster. | `any` | `{}` | no |
