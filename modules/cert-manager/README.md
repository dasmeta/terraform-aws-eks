...
- **Helm Chart**: The cert-manager Helm chart is installed with `crds={enabled=true}` to automatically install Custom Resource Definitions
...
| <a name="input_chart_version"></a> [chart\_version](#input\_chart\_version) | The cert-manager helm chart version | `string` | `"1.20.0"` | no |
...
| <a name="input_configs"></a> [configs](#input\_configs) | Default Helm values to merge with the default cert-manager chart values | <pre>object({<br/>    crds = optional(object({<br/>      enabled = optional(bool, true) # Enable CRD installation<br/>    }), {})<br/>  })</pre> | `{}` | no |
