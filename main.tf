 ...
 *    - Karpenter charts move `1.9.0` -> `1.14.1` and `karpenter-nodes` `0.1.0` -> `0.1.2`. The CRD chart is upgraded before the main chart. If you are coming from a much older release you may still need the `kubectl patch` commands in the 2.20.0 entry below. `ec2:DescribeInstanceStatus` is granted to the controller role, which Karpenter 1.12+ requires for its interruption health checks; without it that code path fails silently with AccessDenied. It is added to the SAME `iam_policy_statements` entry as `iam:ListInstanceProfiles` rather than a second one: the upstream module inlines these into a single policy and AWS caps a managed policy at 6144 characters, a limit that policy already sits close to. A second statement exceeds it, the controller policy then fails to create, and karpenter cannot launch any instance at all. Note 1.9 is the LTS line, so this moves off LTS deliberately, in exchange for the interruption health checks and `Balanced` consolidation this incident needs.
