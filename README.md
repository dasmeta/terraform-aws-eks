...
## Upgrading guide:
 ...
 - from <2.30.0 to >=2.30.0 version, Karpenter gets a stability baseline. **Behaviour changes on upgrade with no configuration change; read this before applying to production.** No state migration is required.
   ...
   - Instance requirements widen from cpu<9 / memory<32Gi to cpu 2-32 / memory 2-128Gi, which deepens the spot candidate pool and lowers interruption frequency. Narrow them again via `karpenter.resource_configs_defaults` if you relied on the old ceiling for cost control.
