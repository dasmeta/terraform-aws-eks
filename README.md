...
| <a name="input_nvidia_gpu_driver"></a> [nvidia\_gpu\_driver](#input\_nvidia\_gpu\_driver) | Configuration block for enabling and customizing the NVIDIA GPU driver installation. | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, "nvidia-device-plugin") # The chart we use has a hard-coded namespace name, so we set our namespace to match it.<br/>    create_namespace = optional(bool, true)<br/>    configs          = optional(any, { "tolerations" : [{ "effect" : "NoSchedule", "key" : "nodetype", "operator" : "Equal", "value" : "gpu" }] })<br/>  })</pre> | `{}` | no |
