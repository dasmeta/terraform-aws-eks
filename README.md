...
| <a name="input_nvidia_gpu_driver"></a> [nvidia\_gpu\_driver](#input\_nvidia\_gpu\_driver) | Configuration block for enabling and customizing the NVIDIA GPU driver installation. | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, "kube-system")<br/>    create_namespace = optional(bool, false)<br/>    configs = optional(any, {<br/>      nodeSelector = {<br/>        nodetype = "gpu"<br/>      }<br/><br/>      tolerations = [<br/>        {<br/>          effect   = "NoSchedule"<br/>          key      = "nodetype"<br/>          operator = "Equal"<br/>          value    = "gpu"<br/>        }<br/>      ]<br/><br/>      affinity = {} # empty affinity<br/>    })<br/>  })</pre> | `{}` | no |
